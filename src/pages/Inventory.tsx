...
const [componentName, setComponentName] = useState('');
  ...
  const handleSubmitRequest = async (item: any) => {
    ...
    if (!item && componentName.trim() === '') return alert('Enter a component name.');
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              ...
              <div className="bg-background border border-dashed border-primary/50 rounded-lg p-5 flex flex-col items-center justify-center gap-4 hover:bg-primary/5 transition-all text-center">
                ...
                {requestingId === 'custom' ? (
                  ...
                  <button onClick={() => { setRequestingId('custom'); setReqQty(1); setReqReason('');
setComponentName(''); }} className="w-full bg-primary/10 text-primary border border-primary/30 py-2 rounded text-xs uppercase tracking-widest hover:bg-primary/20 transition-all mt-2">
                    Request Procurement
                  </button>
                )}
              </div>
              ...
              {filteredItems.map((item) => (
                <div key={item.id} className="bg-card border border-border rounded-lg p-5 flex flex-col gap-4 shadow-sm relative overflow-hidden group">
                  ...
                  {requestingId === item.id ? (
                    ...
                    <button disabled={item.available_quantity === 0} onClick={() => { setRequestingId(item.id); setReqQty(1); setReqReason('');
setComponentName(''); }} className="w-full bg-background border border-border py-2 rounded text-xs uppercase tracking-widest hover:border-primary hover:text-primary transition-all disabled:opacity-50 mt-2">
